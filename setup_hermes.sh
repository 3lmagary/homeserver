...
# ── Install Hermes Host Bridge on Proxmox host ────────
# The Proxmox API /lxc/{vmid}/exec endpoint returns 501 Not Implemented
# when called from inside an LXC container. The host bridge runs directly
# on the Proxmox host and exposes a secure HTTP API for pct exec.
if ! command -v python3 &>/dev/null; then
  log_warn "python3 not found on the Proxmox host; installing for the Host Bridge..."
  apt-get update -qq
...
def run_cmd(command, timeout=60):
    ...
    return {"exit_code": r.returncode, "stdout": r.stdout, "stderr": r.stderr}, r.returncode == 0


def pct_exec(vmid, command, timeout=60):
    return run_cmd(["/usr/sbin/pct", "exec", str(vmid), "--", "bash", "-lc", command], timeout=timeout)


def pct_action(action, vmid, timeout=60):
    return run_cmd(["/usr/sbin/pct", action, str(vmid)], timeout=timeout)
...
def main():
    ...
    print(f"Hermes Host Bridge listening on {BRIDGE_BIND}:{BRIDGE_PORT}", flush=True)
...
log_info "Host Bridge running on port ${HOST_BRIDGE_PORT} — token stored in /etc/hermes-host-bridge.env"
...
def _acquire_vmid_lock(vmid: int):
    """Serialize mutating operations per VMID to prevent race conditions."""
    with VMID_LOCKS_GUARD:
        lock = VMID_LOCKS.setdefault(vmid, threading.Lock())
    lock.acquire()
    return lock
...
def _bridge_request(method: str, path: str, payload: dict = None, timeout: int = 60) -> dict:
    ...
    if not BRIDGE_URL or not BRIDGE_TOKEN:
        raise RuntimeError("Hermes Host Bridge is not configured (HERMES_HOST_BRIDGE_URL / HERMES_HOST_BRIDGE_TOKEN missing).")
    url     = f"{BRIDGE_URL}{path}"
    headers = {"Authorization": f"Bearer {BRIDGE_TOKEN}"}
    try:
        resp = requests.request(method, url, json=payload or {}, headers=headers, timeout=timeout)
    ...
    if resp.status_code >= 400:
        ...
        raise RuntimeError(f"Host bridge {path} failed ({resp.status_code}): {detail}")
    return data
...
def _exec_lxc(vmid: int, command: str, timeout: int = 60) -> str:
    """Run a bash command inside a running LXC via the Proxmox Host Bridge.

    The Proxmox API /lxc/{vmid}/exec endpoint returns 501 Not Implemented
    when called from inside an LXC container. The host bridge (running
    directly on the Proxmox host) executes `pct exec` on our behalf and
    ...
    try:
        ...
        if exit_code not in (0, None):
            raise RuntimeError(
                f"pct exec failed for VMID {vmid} (exit {exit_code}). "
