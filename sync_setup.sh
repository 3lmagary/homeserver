...
systemctl restart docker && sleep 5'
    ...
    echo -e "     - Open Kopia Web UI: ${YELLOW}$KOPIA_URL${NC}"
    echo -e "     - Choose Storage type: ${GREEN}Filesystem${NC}"
    echo -e "     - Enter Directory Path: ${GREEN}/repository${NC}"
    echo -e "     - Enter the Master Repository Password."
    echo -e "  2. ${BOLD}Connect Kopia Client (App or CLI) to this Server:${NC}"
    echo -e "     - Choose connection method: ${GREEN}Kopia Repository Server${NC}"
    echo -e "     - Use Server URL: ${YELLOW}https://kopia.3lmagary.com:443${NC}"
    echo -e "     - (This connects via HTTPS. Nginx Proxy Manager handles the SSL"
